...
function ActivityHeatmap({ data }) {
  ...
  const getCellColor = (total, isFuture) => {
    if (isFuture) return "bg-slate-50 border-dashed border-slate-200/40";
    if (total === 0) return "bg-slate-100 border-slate-200/20";
    if (total <= 1) return "bg-blue-100 border-blue-200";
    if (total <= 2) return "bg-blue-300 border-blue-400";
    if (total <= 4) return "bg-blue-500 border-blue-600";
    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm transition-all hover:shadow-md flex flex-col justify-between h-full min-h-[340px]">
      ...
      <div className="my-6 overflow-x-auto pb-3 -mx-2 px-2 scrollbar-thin">
        <div className="min-w-[640px] relative">
          ...
          {/* Month Labels */}
          <div className="flex text-[10px] font-bold text-slate-400 h-5 relative ml-6">
          ...
          <div className="flex gap-[2px]">
            {/* Y-Axis day labels */}
            <div className="flex flex-col justify-between text-[9px] font-bold text-slate-400 pr-2 pt-[1px] w-6 h-[91px]">
              <span>Sun</span>
              <span>Tue</span>
              <span>Thu</span>
              <span>Sat</span>
              ...
              <div key={`week-${weekIdx}`} className="flex flex-col gap-[2px]">
                {week.map((cell) => {
                  ...
                  return (
                    ...
                    >
                      {/* Tooltip Overlay */}
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 hidden group-hover:block z-50 bg-slate-900 text-white p-3.5 rounded-xl shadow-xl border border-slate-800 text-[10px] font-medium leading-relaxed pointer-events-none transition-all duration-200">
                        ...
                        {cell.isFuture ? (
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="flex justify-between border-t border-slate-800 pt-1 mt-1 font-bold text-white">
      ...
      {/* Footer Info & Legend */}
      <div className="flex items-center justify-between border-t border-slate-100 pt-4 text-[10px] font-semibold text-slate-500">
        ...
        <div className="flex items-center gap-1.5">
          <span>Less</span>
          <span className="w-3.5 h-3.5 rounded-sm bg-slate-100 border border-slate-200/20" />
          <span className="w-3.5 h-3.5 rounded-sm bg-blue-100 border border-blue-200" />
          <span className="w-3.5 h-3.5 rounded-sm bg-blue-300 border border-blue-400" />
          <span className="w-3.5 h-3.5 rounded-sm bg-blue-500 border border-blue-600" />
          <span className="w-3.5 h-3.5 rounded-sm bg-blue-700 border border-blue-800" />
          <span>More</span>
        </div>
      </div>
    </div>
